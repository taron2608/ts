...
# ==================== НАЧАЛО ИЗМЕНЕНИЙ: АКТИВНЫЙ ПИНГ ====================
def active_ping():
...
def get_user(uid):
    ...
    if uid_str not in storage["users"]:
        ...
        save_storage()  # ========== ИЗМЕНЕНИЕ: Сохраняем нового пользователя ==========
...
def cleanup_finished_games():
    ...
    if games_to_remove:
        save_storage()  # ========== ИЗМЕНЕНИЕ: Сохраняем после очистки ==========
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user["state"] = None
    save_storage()  # ========== ИЗМЕНЕНИЕ: Сохраняем состояние ==========
...
async def game_details_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Кнопка пожелания для ВСЕХ участников (включая организатора)
...
async def kick_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if uid in game["players"]:
        try:
            ...
            save_storage()  # ========== ИЗМЕНЕНИЕ: Сохраняем после удаления ==========
...
async def start_game_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    game["pairs"] = pairs
    game["started"] = True
    save_storage()  # ========== ИЗМЕНЕНИЕ: Сохраняем после запуска игры ==========
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user.get("state") == "wait_game_amount":
        ...
        save_storage()  # ========== ИЗМЕНЕНИЕ: Сохраняем игру ==========
    ...
    if user.get("state") and user["state"].startswith("wait_new_amount_"):
        ...
        user["state"] = None
        save_storage()  # ========== ИЗМЕНЕНИЕ: Сохраняем изменения ==========
    ...
    if user.get("state") and user["state"].startswith("wait_wish_want_"):
        ...
        save_storage()  # ========== ИЗМЕНЕНИЕ: Сохраняем пожелания ==========
...
async def handle_start_with_param(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if args and len(args[0]) == 8:
        ...
        user = get_user(user_id)
        user.setdefault("games", []).append(game_id)
        save_storage()  # ========== ИЗМЕНЕНИЕ: Сохраняем нового участника ==========
